    ...
    if (inputPlaca) {
        inputPlaca.addEventListener('blur', function() {
            ...
            if (placa && !validarFormatoPlaca(placa)) {
                mostrarError(this, 'Formato de placa inválido. Ejemplos válidos: ABC123, AB-1234, 123ABC');
            } else {
                limpiarError(this);
            }
        });
...
function inicializarBusquedaRapida() {
    ...
    document.addEventListener('click', function(e) {
        if (!placa.contains(e.target) && !dni.contains(e.target) && !sugerencias.contains(e.target)) {
            ocultarSugerencias();
        }
    });
}
...
function buscarVehiculos(termino, tipoCampo) {
    fetch(`ajax/buscar.php?termino=${encodeURIComponent(termino)}&tipo=${tipoCampo}`)
...
function mostrarSugerencias(resultados, tipoCampo) {
    ...
    resultados.forEach(resultado => {
        ...
        const estadoTexto = resultado.tiene_movimiento_activo ? '🚪 Salida sugerida' : '🚗 Entrada sugerida';
        
        html += `
            <div class="sugerencia-item" onclick="seleccionarVehiculo('${resultado.placa}', '${resultado.dni}', '${resultado.nombre}', '${resultado.tipo_vehiculo}', '${resultado.precio_dia}', '${resultado.tipo_movimiento_sugerido}', '${estado}')">
                <div class="sugerencia-header">
                    <span class="sugerencia-placa">${resultado.placa}</span>
                    <span class="sugerencia-tipo">${resultado.tipo_vehiculo}</span>
                </div>
                <div class="sugerencia-info">
                    ${resultado.nombre} - DNI: ${resultado.dni}
                </div>
                <div class="sugerencia-estado ${estado}">
                    ${estadoTexto} - S/ ${parseFloat(resultado.precio_dia).toFixed(2)}/día
                </div>
            </div>
        `;
    });
 ...
 */
function seleccionarVehiculo(placa, dni, nombre, tipo, precio, tipoMovimiento, estado) {
    // Rellenar campos del formulario
    document.getElementById('placa').value = placa;
    document.getElementById('dni').value = dni;
    document.getElementById('tipo_movimiento').value = tipoMovimiento;
    document.getElementById('precio_dia').value = `S/ ${parseFloat(precio).toFixed(2)}`;
    
    // Mostrar información del vehículo
    document.getElementById('info_nombre').textContent = nombre;
    document.getElementById('info_tipo').textContent = tipo;
    document.getElementById('info_estado').textContent = estado === 'entrada' ? 'Disponible para entrada' : 'En cochera (requiere salida)';
    document.getElementById('info_vehiculo').style.display = 'block';
    
    ocultarSugerencias();
    
    // Mostrar notificación
    mostrarNotificacion(`Vehículo ${placa} seleccionado - ${tipoMovimiento} sugerido`, 'success');
}
